...
if ( isset($_POST['uname']) && isset($_POST['passwd']) && isset($_POST['company']) ) {
    ...
    $_passwd = mysqli_real_escape_string($conn, $upasswd);
    $_nickname = mysqli_real_escape_string($conn, $nickname);
    $_company = mysqli_real_escape_string($conn, $company);
    // 查询当账户名密码一致的结果
    $res = query("select id, uname from user where uname = '$uname'");
    if ( count($res) < 1 ) {
